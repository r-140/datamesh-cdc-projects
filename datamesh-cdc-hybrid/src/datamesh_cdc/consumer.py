...
def process_message(conn, *, topic, partition, offset, key, payload):
    ...
    with conn.cursor() as cur:
        cur.execute(
          ...
          VALUES (%s,%s,%s,%s,%s,%s,%s::jsonb,%s) ON CONFLICT DO NOTHING""",
            (
                ...
                partition,
                offset,
                json.dumps(key, default=json_default),
                table,
                op,
                payload_json,
                payload.get("__source_ts_ms"),
            ),
        )
        if cur.rowcount == 0:
            conn.commit()
            return "duplicate"
        cur.execute(
            """INSERT INTO governance.observed_schemas(source_table,fingerprint,fields)
        ...
        try:
            ...
            cur.execute(
                """INSERT INTO governance.projection_failures
