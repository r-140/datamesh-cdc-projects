...
{Colors.OKCYAN}{Colors.BOLD}
  ...
  \___ \| | | || | | | | | | | |\/| | |_) | | | | ' / 
   ___) | |_| || |_| | | |_| | |  | |  __/| |_| | . \ 
  |____/|____/ |____/  |____/|_|  |_|_|    \___/|_|\_\
{Colors.END}
  {Colors.OKGREEN}CDC Data Generator — Watch changes flow in real-time{Colors.END}
""")
...
def update_orders(count: int = 1) -> list:
    ...
    for row in rows:
        ...
        result = execute(
            "orders",
            "UPDATE orders SET status = %s, updated_at = NOW() WHERE id = %s RETURNING id, customer_id, total_amount, status, updated_at",
            (new_status, row["id"]),
            fetch=True,
        )
        if result:
...
def get_kafka_offsets():
    """Get current Kafka offsets for CDC topics."""
...
def wait_for_cdc_sync(max_wait: int = 15):
    """Wait briefly for CDC to propagate, then show DWH state."""
    ...
    try:
        while True:
            ...
            if table == "orders" or table is None:
                orders = insert_orders(random.randint(1, 3))
            if table == "customers" or table is None:
                customers = insert_customers(random.randint(1, 2))
            # Occasionally do updates/deletes
...
Examples:
  ...
  %(prog)s --mode verify
    ...
    if args.mode == "batch":
        if args.table in ("orders", "both"):
            insert_orders(args.count)
        if args.table in ("customers", "both"):
            insert_customers(args.count)
        ...
        if args.table in ("orders", "both"):
            update_orders(args.count)
        if args.table in ("customers", "both"):
            update_customers(args.count)

    elif args.mode == "delete":
        if args.table in ("orders", "both"):
            delete_orders(args.count)
        if args.table in ("customers", "both"):
            delete_customers(args.count)
        ...
        return  # verify already shown on stop
