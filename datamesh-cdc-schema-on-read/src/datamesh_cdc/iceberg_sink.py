"""
Iceberg Sink — SCHEMA-ON-READ (Bronze as JSON/VARIANT).

Bronze layer stores all CDC events with a FIXED schema:
  - _cdc_key, _payload (JSON string), _cdc_op, _cdc_ts_ms, etc.

New fields in source simply appear as new keys in JSON.
Pipeline never breaks. Schema is applied at Silver/Gold read time.
"""
...
class BronzeIcebergSink:
    """Bronze sink with fixed schema. All payloads stored as JSON."""
    ...
    def __init__(self, catalog_uri: str = "http://localhost:8181",
        ...
        logger.info(f"BronzeIcebergSink initialized: {catalog_uri}")
    ...
    def append_event(self, table_name: str, event: dict, namespace: str = "bronze") -> dict:
        """Append CDC event with JSON payload. Never fails due to schema changes."""
        ...
        # In production: append via PyIceberg or Spark
