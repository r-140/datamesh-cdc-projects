"""
Pipeline Manager — Hybrid.

Manages CDC pipelines with Bronze JSON + Silver DDL validation.
"""
...
class PipelineManager:
    ...
    def validate_silver_for_pipeline(self, pipeline_id: str, expected_fields: List[str]) -> dict:
        """Validate Silver DDL fields against source schema."""
        pipeline = self.pipelines.get(pipeline_id)
        if not pipeline:
            return {"error": f"Pipeline {pipeline_id} not found"}
        subject = f"{pipeline.config.source_topic}-value"
        return self.schema_manager.validate_silver_ddl(subject, expected_fields)
...
class SelfServeAPI:
    ...
    def validate_silver_request(self, request: dict) -> dict:
        """Self-serve validation of Silver DDL."""
        pipeline_id = request.get("pipeline_id")
        fields = request.get("expected_fields", [])
        if not pipeline_id or not fields:
            return {"error": "pipeline_id and expected_fields required"}
        return self.manager.validate_silver_for_pipeline(pipeline_id, fields)
