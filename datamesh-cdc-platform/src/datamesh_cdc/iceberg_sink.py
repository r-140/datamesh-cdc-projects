"""
Iceberg CDC Sink with schema evolution support.

Features:
- Append-only raw tables
- Schema evolution (ADD COLUMN, TYPE WIDENING)
- SCD Type 2 views via Trino
- Time-travel queries
"""
...
class IcebergCDCSink:
    """Sink for CDC data into Iceberg tables with schema evolution."""
    ...
    def create_cdc_table(
        ...
        """
        Create append-only CDC table with partitioning by ingestion day.
        ...
        all_fields = list(schema.fields) + cdc_fields
        final_schema = Schema(*all_fields)
    ...
    def apply_schema_evolution(
        self,
        table_name: str,
        new_avro_schema: dict,
        namespace: str = "raw"
    ) -> Dict[str, Any]:
        """
        Apply schema evolution to existing Iceberg table.
    ...
    def create_scd2_view(self, table_name: str, namespace: str = "raw") -> str:
        """
        Generate SQL for SCD Type 2 view over append-only CDC table.
        ...
        FROM (
            SELECT *,
                ROW_NUMBER() OVER (
                    PARTITION BY id 
                    ORDER BY _cdc_ts_ms DESC, _cdc_op DESC
                ) AS rn
    ...
    def create_time_travel_view(self, table_name: str, namespace: str = "raw") -> str:
        """Generate SQL for time-travel view."""
        sql = f"""
        CREATE OR REPLACE VIEW {namespace}.{table_name}_history AS
        SELECT *,
            LAG(_cdc_ts_ms) OVER (PARTITION BY id ORDER BY _cdc_ts_ms) AS prev_ts
        FROM {namespace}.{table_name}
        """
        return sql
    ...
    def _avro_to_iceberg_schema(self, avro_schema: dict) -> Schema:
        ...
        for field in avro_schema.get('fields', []):
            ...
            if isinstance(field_type, list):
                # Union type: ["null", "string"] -> optional
    ...
    def _is_safe_widening(self, old_type, new_type) -> bool:
        ...
        allowed = widening.get(type(old_type), [])
        return type(new_type) in allowed

    def get_table_metadata(self, table_name: str, namespace: str = "raw") -> Dict:
        """Get table metadata including schema history."""
        identifier = f"{namespace}.{table_name}"
        table = self.catalog.load_table(identifier)
